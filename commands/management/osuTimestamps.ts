...
export default {
    ...
    category: "management",
    help: {
        description: "Enable or disable the detection of osu! timestamps in messages",
    ...
    run: async (bot: Client, command: CommandInteraction, args: string[]) => {
        ...
        if (typeof command.member?.permissions == "string") return;
            ...
            const status = command.options.get("status") ? (command.options.get("status")?.value == "true") : true;
