		...
		.then((beatmaps) => {
			...
			for (let bsId of beatmapsets) {
				if (
					beatmaps.some(
						(beatmap) =>
							beatmap.beatmapSetId === bsId &&
							rankedTypes.indexOf(beatmap.approvalStatus) !== -1
					)
				) {
					...
					beatmaps.some(
						(beatmap) =>
							beatmap.beatmapSetId === bsId &&
							unrankedTypes.indexOf(beatmap.approvalStatus) !== -1
					)
				) {
			...
			let mappingAgeUnix = new Date(
				new Date().getTime() - oldestMap.getTime()
			);
			let mappingAge = parseDate(mappingAgeUnix);
			let latestMapsetId = Math.max(...beatmapsets);
			let latestMapset = beatmaps.find(
				(beatmap) => beatmap.beatmapSetId == latestMapsetId
			);
			...
			let latestMapsetURL =
				"https://osu.ppy.sh/beatmapsets/" + latestMapsetId;
			let imageCover =
			...
			let embed = new MessageEmbed()
				...
				.addField(
					"Playcount & Favorites",
					(value = "▶ " + totalPlaycount + "  💖 " + favoritesCount),
					true
				)
				.addField(
					"Latest Map",
					`[${latestMapsetArtist} - ${latestMapsetTitle}](${latestMapsetURL})`,
					false
				)
				.setImage(imageCover)
				.setTimestamp();
