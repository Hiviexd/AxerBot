export default class Accuracy {
	 ...
	 * be automatically calculated to be the closest to the given acc percent.
	 *
	 ...
	 * @param {number} [values.nmiss] The amount of misses achieved.
	 */
	...
	/**
	 * Computes the accuracy value (0.00 - 100.00).
	 *
	 ...
	 * @returns {number} The accuracy value ranging from `0.00` to `100.00`.
	 */
	value(nobjects?: number) {
		let n300 = this.n300;
		if (n300 < 0) {
			if (!nobjects) {
				throw new TypeError(
		...
		let res =
			...
			((nobjects || 0) * 300.0);
		return Math.max(0, Math.min(res, 1.0)) * 100;
	}
