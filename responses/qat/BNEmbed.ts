...
export default {
    send: (
        ...
        const latestNom = activity.data.uniqueNominations[activity.data.uniqueNominations.length - 1];
        ...
        let e = new MessageEmbed()
            ...
            if (qatUser.data.natDuration) {
                e.addField("NAT For", `${calculateDuration(qatUser.data.natDuration)}`, true);
            } else {
                e.addField("\u200b", "\u200b", true);
            }
            ...
            e.addFields(                
                ...
                {
                    name: "Mappers",
                    value: `${getUniqueMappersNumber(activity).toString()} (${Math.floor(getUniqueMappersNumber(activity) / activity.data.uniqueNominations.length * 100)}%)`,
                    inline: true,
                },
                {
