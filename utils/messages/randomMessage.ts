...
export default async function randomMessage(message: Message, bot: any ) {
    ...
    if (guild.fun.enable == true && message.content.toUpperCase().includes("AXER") || message.mentions.users.has(bot)) {
        if (guild.fun.mode == "default") {
            const quotes = await parseTextFile(path.resolve(__dirname+"/../../data/axer.txt"))
